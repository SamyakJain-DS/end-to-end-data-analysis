            ...
            try:
                ...
                while True:
                    ...
                    if new_height == old_height:
                        total_results = min(
                            int(brand_driver.find_element(by=By.CLASS_NAME, value='pg-prf-head').text.split(" ")[0].replace(",","")), 1020
                        )
                ...
                logging.error(f"Failed To Fetch The {brand} HTML Page ({category}) : {e}")
                logging.warning(f"Added {brand} ({category}) To Failed Cases.")
    ...
    for attempt in range(max_attempts):
        ...
        for category in new_htmls.keys():  # merge this new htmls with htmls
            for brand in new_htmls[category].keys():
                categories_htmls[category][brand] = new_htmls[category][brand]

    return categories_htmls

categories = ("laptops", "mobiles", "tablets")
